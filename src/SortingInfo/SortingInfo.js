...
export default function SortingInfo() {
  ...
  return (
    <div className="sorting-info-card">
      ...
      <p><span className="info">Array type:</span>{currentArrayInfo.name}</p>
      <p><span className="info">Algorithm name:</span>{currentAlgorithmInfo.name}</p>
      <p><span className="info">Sorting complexity:</span>{currentAlgorithmInfo.complexity}</p>
      <p><span className="info">Count of inversions:</span>{inversions ?? "-"}</p>
      <p><span className="info">Count of comparisons:</span>{comparisons ?? "-"}</p>
    </div>
  );
}
